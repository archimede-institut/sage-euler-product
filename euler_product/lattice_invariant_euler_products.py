...
def get_euler_products(q, s, f_init, h_init, nb_decimals=100, big_p=300, verbose=2, with_laTeX=0):
    ...
    EXAMPLES::
        ...
        0.9671040753637981066150556834173635260473412207450092130719978569438733967843271277395717230016746853806050215621235810749643636399725665325875376146914709362753787689855429317947529895445140974478),
        ...
        0.7071813747951674302088659938984504109243584468119496848353517677901518159831128643782536704398941052120208041311403202957250160794697319584608281454011743387515885835706146696365506658500107821229)))      
        sage: get_euler_products(8, 1, 1-2*x-7*x^2-4*x^3, 1-2*x+x^2, 110, 50, 0, 0)  # doctest: +NORMALIZE_WHITESPACE
        ((frozenset({1}), frozenset({3}), frozenset({5}), frozenset({7})),
        ((0.95694534785160118343696705727389182875317497729139147890543260424601701644488885948144051203907950843120693625020815416475404367245118705539024048071745051900627666754627660,
        0.95694534785160118343696705727389182875317497729139147890543260424601701644488885948144051203907950843120698715116914094815616983623707385617396957220137053238960253655524739),
        (-1.1744975454490034177155999666151209160615842743206296883125115013500947685793723674107330894468690657084850070249226110245398822929600793504268863625348966487119718808256607,
        -1.1744975454490034177155999666151209160615842743206296883125115013500947685793723674107330894468690657084850694977072728046467252979967979487933249698024192018658408556147986),
        (0.41303522430115813462933064104403132614973953141027630692477294245200590806656802414019031361757979375573715775187373750687146071788259978947093502284448731978739395024067003,
        0.41303522430115813462933064104403132614973953141027630692477294245200590806656802414019031361757979375573717972166689137488171648065043534619360359980646057811497431572380265),
        (0.73440531676250743047848787987972815324702590578219161532691471695315326661191699963756307237940693554033848218351377182525147759457886448953074386173336906260388598561645334,
        0.73440531676250743047848787987972815324702590578219161532691471695315326661191699963756307237940693554033852124733175619670365060797768371613466902929945644571107584535601747)))
    """
    start = timer()
    # assert F[0] = H[0] = 1
    #  Compute the structural invariants:

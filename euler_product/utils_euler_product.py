...
class ComponentStructure():
    ...
    - ``getr_A_Kt``: a method. ??Given a cyclic subgroup K, a lattice invariant class A and a positive integer t, form the list of cyclic subgroups L containing K and such that the set of t-th powers of elements of L is exactly the subgroup generated by A. Returns the sum moebius_function(|L|/|K|)/(|L|/|K|).
    - ``get_CA_Km``: a method.
    - ``get_L_values``: a method.
    - ``get_CA_Km_F_sur_H``: a method.
    - ``get_gamma``: a method.
